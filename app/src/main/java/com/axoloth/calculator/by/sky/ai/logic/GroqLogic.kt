...
object GroqLogic {
    ...
    suspend fun getAiExplanationStream(
        ...
        try {
            ...
            val encryptedKey = remoteConfig.getString("Groq_AI")
            ...
            val jsonBody = JSONObject().apply {
                put("model", "llama-3.3-70b-versatile") // Atau model Groq lain yang Abang mau
                put("messages", org.json.JSONArray().apply {
                    ...
                    put(JSONObject().apply {
                        put("role", "user")
                        put("content", "Explain: $expression")
                    })
                })
                put("stream", true)
                put("max_tokens", 500)
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    withContext(Dispatchers.Main) { onError("Groq Error: ${response.code}", response.code) }
                    return@withContext
                }
            ...
            withContext(Dispatchers.Main) { onError(e.message ?: "Groq Error", -1) }
        }
    }
}
