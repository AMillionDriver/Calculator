...
object GptLogic {
    ...
    suspend fun getAiExplanationStream(
        ...
        try {
            ...
            val encryptedKey = remoteConfig.getString("GPT_AI")
            ...
            val jsonBody = JSONObject().apply {
                put("model", "gpt-4o-mini") // Model murah dan cepat dari OpenAI
                put("messages", org.json.JSONArray().apply {
                    ...
                    put(JSONObject().apply {
                        put("role", "user")
                        put("content", "Explain this calculation: $expression")
                    })
                })
                put("stream", true)
                put("max_tokens", 500)
            ...
            client.newCall(request).execute().use { response ->
                if (!response.isSuccessful) {
                    withContext(Dispatchers.Main) { onError("GPT Error: ${response.code}", response.code) }
                    return@withContext
                }
            ...
            withContext(Dispatchers.Main) { onError(e.message ?: "GPT Connection Error", -1) }
        }
    }
}
